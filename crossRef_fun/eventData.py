...
class eventData:
    ...
    def __init__(self, **kwargs):
        ...
        if "outputFile" in kwargs:
            ...
            self.outputFile = 'test.json' # file that the output from the query are saved as, should be json
        ...
        # Internal varaibles
        self.queryUrl = '' # displays the command executed; note that the acutal call is done with the requests package
        ...
        self.events = eventRecord() # event record to store collected events

    def clear(self):
        ''' Clear current query results (except for the cursor) '''

        # clear query parameters
        self.params = {}

        # clear output and properties
        self.events = eventRecord()

    def buildQuery(self, filters, quiet=False, cursor = None, scholix = False, deleted = False):
        ...
        filters: dict
            ...
            kwargs can contain:
                ...
                source # source ID
                scholix # true/false returns only results included in scholix (but not necessarily scholix format)
                facets # a facet (type of data to return summary stats for)

                See https://www.eventdata.crossref.org/guide/service/query-api/ for online documentation

        scholix: boolean
            if True, uses the scholix endpoint. Note that this must also be specified
            in runQuery.

        deleted: boolean
            if True, uses the 'deleted' endpoint. Must also be specified in runQuery

        '''

        # Clear the results of previous queries
        self.clear()
        ...
        for k in filters:
            if k in allowedArgs:
                ...
                print('non-standard filter found, ' + k + ', ' + str(filters[k]))
        ...
        if not(quiet):
            ...
            if scholix:
                self.queryUrl = 'https://api.eventdata.crossref.org/v1/events/scholix?'
            elif deleted:
                self.queryUrl = 'https://api.eventdata.crossref.org/v1/events/deleted?'
            else:
    ...
    def runQuery(self, retry=1, quiet=False, scholix = False, deleted = False):
        '''
        ...
        scholix: boolean
            if True, queries the Scholix endpoint

        deleted: boolean
            if True, queries the deleted endpoint
        ...
        # the query URL
        if scholix:
            url = "https://api.eventdata.crossref.org/v1/events/scholix"
        elif deleted:
            url = "https://api.eventdata.crossref.org/v1/events/deleted"
        else:
    ...
    def getNextPage(self):
        ...
        if self.cursor in ("-1", None):
            ...
            if self.outputFile[-5:] == ".json":
                ...
                self.outputFile = self.outputFile[:-5]+ str(self.pageCount).zfill(3)+ ".json"
            else:
                self.outputFile = self.outputFile[:-5]+ str(self.pageCount).zfill(3)
    ...
    def getAllPages(self, maxPages, filters, fileprefix):
        '''
        ...
        fileprefix - will add a 4 digit number and '.json'
...
if __name__=="__main__":
    ...
    fl = {'from-collected-date': '2021-02-01', 'rows': 100, 'until-collected-date': '2021-02-04'}
    # ed.buildQuery(fl, deleted = True)
    # ed.runQuery(retry = 5, deleted = True)

    ed.getAllPages(5, fl, 'demo')
